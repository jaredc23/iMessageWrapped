...
const MessagesTimelineChart = ({ formatted, height = 400 }) => {
  ...
  // Convert labels into a numeric x value similar to EmojiTimeline: month + fractional day
  const displayed = formatted.map(row => {
    const out = { label: row.label, value: Number(row.value) || 0 };
      try {
      ...
      <div style={{ height: '100%', width: '100%', background: 'transparent' }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={displayed} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
              ...
              <Area type="linear" dataKey="value" name="Messages" stroke={stroke} fill={fill} strokeWidth={2} dot={false} isAnimationActive={true} animationBegin={0} animationDuration={900} />
            </AreaChart>
          </ResponsiveContainer>
      </div>
    </div>
